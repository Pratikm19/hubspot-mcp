...
## Extending the official HubSpot MCP server

The official HubSpot MCP server is currently in beta. This repo extends it with a logic layer built for RevOps practitioners — adding Gemini CLI support, a pre-loaded context file, a 35-prompt skill library, and 5 end-to-end workflow recipes.

| Feature | HubSpot Official MCP | pratikm19/hubspot-mcp |
|---|---|---|
| Read CRM data | ✅ | ✅ |
| Write to CRM | ✅ | ✅ |
| Gemini CLI support | ❌ | ✅ |
| Claude Code support | ✅ | ✅ |
| Pre-loaded property names | ❌ | ✅ via GEMINI.md |
| Discovery gate | ❌ | ✅ via GEMINI.md |
| RevOps prompt library | ❌ | ✅ 35 prompts |
| End-to-end workflow recipes | ❌ | ✅ 5 recipes |
| Multi-portal config | ❌ | ✅ |
| Windows setup guide | ❌ | ✅ |
| Job-to-be-done organisation | ❌ | ✅ |
| Token overhead reduction | ❌ | ✅ via context + explicit prompts |
| Free | ✅ | ✅ |
...
The official HubSpot MCP server exposes the full CRM surface area. That's the right design for a general-purpose connector — but it means your agent has to explore the schema before every query, adding overhead before it even starts working.

This repo adds a context layer on top that eliminates that exploration step.

`GEMINI.md` pre-loads into every Gemini CLI session automatically. It contains:

- Exact property names for deals, contacts, and companies
- Deal stage internal values
- Lifecycle stage internal values
- Behaviour rules — use deal IDs for writes, always specify HubSpot as the source, use exact property names in queries

The result: the agent knows what to ask and how to ask it before the first query runs. No schema discovery. No property lookup chains. Queries that would take 2 minutes with a vague prompt run in seconds with an explicit one.

The `SKILL.md` prompt library extends this further — 35 prompts with exact property names and filters baked in, organised by RevOps use case rather than API structure.
